    ...
    while($row=mysqli_fetch_assoc($sql)){
        ...
        if($date==$return_date){
            $query=mysqli_query($conn,"SELECT customer_details.*,order_details.* from customer_details join order_details on customer_details.id=order_details.User_id
             WHERE order_details.approve='yes' and order_details.return_date='$date';");
             if(mysqli_num_rows($query)>0){
                 while($row_u=mysqli_fetch_assoc($query)){
                    ...
                    $mail->isHTML(true);
                    $mail->Subject="Book issue Expires today!";
                ...
                <p style=' text-align: center;
                    ...
                    padding-top: 100px;'>Hi, $name, your issue of $bookname expires today. Please ensure that the book is in perfect condtion and return it to our delivery agent. Thank You. </p></div>";
